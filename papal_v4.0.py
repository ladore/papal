...
class MarketSignal:
    ...
    def reliability_weight(self) -> float:
        return self.liquidity_weight * self.spread_weight
...
class SourceReliability:
    ...
    def observe(self, signal_quality: float, weight: float) -> None:
        bounded_quality = clamp_unit_interval(signal_quality)
        bounded_weight = max(weight, 0.0)
        self.alpha += bounded_quality * bounded_weight
        self.beta += (1 - bounded_quality) * bounded_weight
...
class ResonanceBayesianTracker:
    ...
    def __init__(
        ...
        self.detailed_console = detailed_console
    ...
    def add_event(self, event: EvidenceEvent) -> None:
        ...
        source_state = self._source_reliability(event.source)
        effective_confidence = clamp_unit_interval(
            event.confidence * (0.50 + source_state.mean / 2)
        )
        for hypothesis in self.hypotheses:
            ...
            weighted_lr = self._temper_likelihood_ratio(
                dependency_adjusted_lr, effective_confidence
            )
        ...
        source_state.observe(event_signal_quality(event), evidence_weight(event))

        if self.verbose:
            print(f"[{event.date.date()}] {event.title}")
            print_console_note("Evidence", event.rationale)
            print_console_note(
                "Source reliability",
                f"{event.source}: {format_percent(source_state.mean)} "
                f"({source_reliability_phrase(source_state.mean)}); effective confidence "
                f"{format_percent(effective_confidence)}.",
            )
            ...
            if posterior_rows:
                ...
                print_console_note(
                    "Current leader",
                    f"{top['label']} at {format_percent(cast(float, top['posterior']))} "
                    f"({top['interpretation']}).",
                )
            print_compact_rows(compact_posterior_rows(posterior_rows))
            if self.detailed_console:
                print("\nDetailed posterior table:")
    ...
    def add_market_signal(self, signal: MarketSignal) -> None:
        ...
        if self.verbose:
            print(
                ...
                f"  yes={format_percent(signal.yes_odds)}; "
                f"reliability weight={format_percent(signal.reliability_weight)} "
                f"({liquidity_phrase(signal.liquidity_weight)}, {spread_phrase(signal.spread)})."
            )
        ...
        return [
            {
                ...
                "source_reliability": round(
                    self._source_reliability(event.source).mean, 4
                ),
    ...
    def market_table(self) -> list[dict[str, object]]:
        ...
        for signal in self.market_signals:
            weighted_signal = (signal.yes_odds - 0.5) * signal.reliability_weight
            rows.append(
                {
                    ...
                    "bid": signal.bid,
                    "ask": signal.ask,
                    "spread_weight": round(signal.spread_weight, 3),
                    "spread_read": spread_phrase(signal.spread),
                    "reliability_weight": round(signal.reliability_weight, 3),
    ...
    def show_polymarket_summary(self) -> None:
        ...
        print_compact_rows(compact_market_rows(self.market_table()))
        if self.detailed_console:
            print("\nDetailed market signal table:")
        ...
        if opinion_rows:
            ...
            print_compact_rows(compact_pool_rows(opinion_rows))
            if self.detailed_console:
                print("\nDetailed market opinion pool:")
        ...
        if filtered_rows:
            ...
            print_compact_rows(compact_kalman_rows(filtered_rows))
            if self.detailed_console:
                print("\nDetailed Kalman table:")
        ...
        if arbitrage_rows:
            ...
            print_compact_rows(arbitrage_rows)
            if self.detailed_console:
                print("\nDetailed consistency checks:")
                print(format_table(arbitrage_rows))

    def show_source_reliability_summary(self) -> None:
        rows = self.source_reliability_table()
        if not rows:
            print("\nNo source reliability observations yet.")
            return
        print("\n=== Source Reliability ===")
        print_console_note(
            "What this is",
            "Each source gets a lightweight Beta reliability score. Stronger, clearer evidence "
            "nudges the source upward; weak or low-confidence entries keep it cautious.",
        )
        print_compact_rows(rows)
        if self.detailed_console:
            print("\nDetailed source reliability table:")
            print(format_table(rows))

    def source_reliability_table(self) -> list[dict[str, object]]:
        rows = []
        for source, reliability in sorted(self.source_reliability.items()):
            lower, upper = reliability.interval()
            rows.append(
                {
                    "source": source,
                    "mean_reliability": round(reliability.mean, 4),
                    "credible_interval": f"{lower:.3f}-{upper:.3f}",
                    "read": source_reliability_phrase(reliability.mean),
                    "alpha": round(reliability.alpha, 3),
                    "beta": round(reliability.beta, 3),
                }
            )
        return rows
    ...
    def market_opinion_pool_table(self, model_weight: float = 1.0) -> list[dict[str, object]]:
        ...
        for hypothesis in self.hypotheses:
            ...
            for signal in self.market_signals:
                ...
                reliability = signal.reliability_weight * relevance
                if reliability <= 0:
                    continue
    ...
    def run_posterior_predictive_checks(
        self, simulations: int = 200, seed: int = 991
    ) -> PosteriorPredictiveResult:
        if not self.posterior_history:
            result = PosteriorPredictiveResult(
                [], "No posterior history available for posterior predictive checks."
            )
            if self.verbose:
                self.print_posterior_predictive_result(result)
            return result
        ...
        for hypothesis in self.hypotheses:
            observed_support = [
                point.likelihood_ratio / (1 + point.likelihood_ratio)
                for point in self.posterior_history
                if point.hypothesis == hypothesis.key
            ]
            if not observed_support:
                continue
            belief = self.belief_states[hypothesis.key]
            simulated_means = [
                rng.betavariate(belief.alpha, belief.beta) for _ in range(simulations)
            ]
            observed_mean = sum(observed_support) / len(observed_support)
            simulated_mean = sum(simulated_means) / len(simulated_means)
            gap = abs(observed_mean - simulated_mean)
            rows.append(
                {
                    "hypothesis": hypothesis.key,
                    "observed_support_mean": round(observed_mean, 4),
                    "predictive_mean": round(simulated_mean, 4),
                    "absolute_gap": round(gap, 4),
                    "read": posterior_predictive_phrase(gap),
                }
            )

        if rows:
            max_gap = max(cast(float, row["absolute_gap"]) for row in rows)
            summary = (
                f"Posterior predictive max gap is {max_gap:.3f}; "
                f"{posterior_predictive_phrase(max_gap)}."
            )
        else:
            summary = "Posterior predictive checks produced no comparable rows."
        result = PosteriorPredictiveResult(rows, summary)
        if self.verbose:
            self.print_posterior_predictive_result(result)
        return result

    def print_posterior_predictive_result(
        self, result: PosteriorPredictiveResult
    ) -> None:
        print("\n=== Posterior Predictive Checks ===")
        print_console_note(
            "What this checks",
            "The model samples from each current belief distribution and compares those samples "
            "with the support actually implied by observed likelihood ratios.",
        )
        print(result.summary)
        if result.rows:
            print_compact_rows(compact_posterior_predictive_rows(result.rows))
            if self.detailed_console:
                print("\nDetailed posterior predictive table:")
    ...
    def plot_phase_space(
        ...
        if plt is None:
            print("Skipping phase-space plot because matplotlib is not installed.")
            return
        config = config or DynamicalAnalysisConfig()
        snapshots = self._posterior_snapshots()
        embedded = self._embedded_state_vectors(snapshots, config)
        if len(embedded) < 2 or len(embedded[0]) < 2:
            print("No phase-space trajectory to plot.")
            return

        x_values = [point[0] for point in embedded]
        y_values = [point[1] for point in embedded]
        plt.figure(figsize=(7, 6))
        plt.plot(x_values, y_values, marker="o", linewidth=2)
        for index, (x_value, y_value) in enumerate(zip(x_values, y_values)):
            plt.annotate(str(index), (x_value, y_value), textcoords="offset points", xytext=(5, 5))
        plt.title("Phase-Space Portrait: Posterior Trajectory")
        ...
        if output_path:
            plt.savefig(output_path, dpi=160)
            print(f"Saved phase-space plot to {output_path}")
        else:
            plt.show()
        ...
        for prior_shift in config.prior_shifts:
            ...
            for likelihood_scale in config.likelihood_scales:
                for confidence_scale in config.confidence_scales:
                    trial = ResonanceBayesianTracker(
                        shifted_hypotheses, verbose=False, detailed_console=False
                    )
                    for event in self.events:
    ...
    def print_sensitivity_result(self, result: SensitivityResult) -> None:
        ...
        if result.rows:
            ...
            print_compact_rows(compact_sensitivity_rows(result.rows))
            if self.detailed_console:
                print("\nDetailed posterior robustness table:")
                print(format_table(result.rows))
        if result.classification_rows:
            ...
            print_compact_rows(compact_classification_rows(result.classification_rows))
            if self.detailed_console:
    ...
    def print_chaos_stability_result(self, result: ChaosStabilityResult) -> None:
        ...
        if result.step_rows:
            ...
            print_compact_rows(compact_step_rows(result.step_rows))
            if self.detailed_console:
        ...
        if result.hypothesis_rows:
            ...
            print_compact_rows(compact_hypothesis_behavior_rows(result.hypothesis_rows))
            if self.detailed_console:
                print("\nDetailed hypothesis behavior table:")
        ...
        if result.recurrence_rows:
            ...
            print_compact_rows(result.recurrence_rows)
            if self.detailed_console:
                print("\nDetailed recurrence table:")
        ...
        if result.lyapunov_rows:
            ...
            print_compact_rows(result.lyapunov_rows)
            if self.detailed_console:
                print("\nDetailed Lyapunov table:")
        ...
        if result.dimension_rows:
            ...
            print_compact_rows(result.dimension_rows)
            if self.detailed_console:
                print("\nDetailed correlation dimension table:")
        ...
        if result.surrogate_rows:
            ...
            print_compact_rows(result.surrogate_rows)
            if self.detailed_console:
                print("\nDetailed surrogate table:")
    ...
    def export_report(
        ...
        posterior_predictive_result: PosteriorPredictiveResult | None = None,
        bayesian_backtest_result: BayesianBacktestResult | None = None,
    ) -> None:
        path = Path(output_path)
        path.parent.mkdir(parents=True, exist_ok=True)
        ...
        lines = [
            ...
            "## Source Reliability",
            "",
            markdown_table(self.source_reliability_table())
            if self.source_reliability
            else "No source reliability rows.",
            "",
            "## Market Signals",
            "",
        ...
        if posterior_predictive_result is not None:
            lines.extend(
                [
                    "",
                    "## Posterior Predictive Checks",
                    "",
                    posterior_predictive_result.summary,
                    "",
                    markdown_table(posterior_predictive_result.rows)
                    if posterior_predictive_result.rows
                    else "No posterior predictive rows.",
                ]
            )

        if bayesian_backtest_result is not None:
            lines.extend(
                [
                    "",
                    "## Synthetic Bayesian Backtest",
                    "",
                    bayesian_backtest_result.summary,
                    "",
                    "### Runs",
                    "",
                    markdown_table(bayesian_backtest_result.rows)
                    if bayesian_backtest_result.rows
                    else "No backtest run rows.",
                    "",
                    "### Calibration",
                    "",
                    markdown_table(bayesian_backtest_result.calibration_rows)
                    if bayesian_backtest_result.calibration_rows
    ...
    def _source_reliability(self, source: str) -> SourceReliability:
        if source not in self.source_reliability:
            self.source_reliability[source] = SourceReliability(source)
...
def print_compact_rows(rows: Sequence[Mapping[str, object]], indent: str = "  - ") -> None:
    if not rows:
        print("  (none)")
        return
    for row in rows:
        print(indent + compact_row_text(row))


def compact_row_text(row: Mapping[str, object]) -> str:
    return "; ".join(f"{key}: {_format_cell(value)}" for key, value in row.items())


def compact_posterior_rows(rows: Sequence[Mapping[str, object]]) -> list[dict[str, object]]:
    return [
        {
            "belief": row["label"],
            "prob": format_percent(cast(float, row["posterior"])),
...
def compact_market_rows(rows: Sequence[Mapping[str, object]]) -> list[dict[str, object]]:
    return [
        {
            "market": shorten_text(cast(str, row["question"]), 62),
            "yes": format_percent(cast(float, row["yes_odds"])),
            "weight": format_percent(cast(float, row["reliability_weight"])),
            "read": row["signal_read"],
        }
        for row in rows
    ]


def compact_pool_rows(rows: Sequence[Mapping[str, object]]) -> list[dict[str, object]]:
    return [
        {
            "belief": row["hypothesis"],
            "model": format_percent(cast(float, row["model_probability"])),
            "pooled": format_percent(cast(float, row["pooled_probability"])),
            "shift": format_signed_percent(cast(float, row["shift"])),
        }
        for row in rows
    ]


def compact_kalman_rows(rows: Sequence[Mapping[str, object]]) -> list[dict[str, object]]:
    return [
        {
            "market": shorten_text(cast(str, row["question"]), 62),
            "observed": format_percent(cast(float, row["latest_observed"])),
            "filtered": format_percent(cast(float, row["filtered_probability"])),
            "read": row["read"],
        }
        for row in rows
    ]
...
def compact_posterior_predictive_rows(
    rows: Sequence[Mapping[str, object]]
) -> list[dict[str, object]]:
    return [
        {
            "belief": row["hypothesis"],
            "observed": format_percent(cast(float, row["observed_support_mean"])),
            "predicted": format_percent(cast(float, row["predictive_mean"])),
            "gap": format_percent(cast(float, row["absolute_gap"])),
            "read": row["read"],
        }
        for row in rows
    ]
...
def compact_sensitivity_rows(rows: Sequence[Mapping[str, object]]) -> list[dict[str, object]]:
    return [
        {
            "belief": row["hypothesis"],
            "range": (
                f"{format_percent(cast(float, row['min_posterior']))}-"
                f"{format_percent(cast(float, row['max_posterior']))}"
            ),
            "spread": format_percent(cast(float, row["spread"])),
    ...
    return [
        {
            ...
            "share": format_percent(cast(float, row["share"])),
            "runs": row["count"],
        }
        for row in rows
    ]
...
def compact_step_rows(rows: Sequence[Mapping[str, object]]) -> list[dict[str, object]]:
    return [
        {
            "move": f"{row['from']} -> {row['to']}",
            "size": row["movement_norm"],
...
def compact_hypothesis_behavior_rows(
    rows: Sequence[Mapping[str, object]]
) -> list[dict[str, object]]:
    return [
        {
            "belief": row["hypothesis"],
            "latest_delta": format_signed_decimal(cast(float, row["latest_delta"])),
            "stability": row["stability_read"],
            "direction": row["direction_read"],
        }
        for row in rows
    ]


def compact_backtest_rows(rows: Sequence[Mapping[str, object]]) -> list[dict[str, object]]:
    return [
        {
            "run": row["run"],
            "truth": row["true_hypothesis"],
            "prob": format_percent(cast(float, row["predicted_probability"])),
            "brier": row["brier"],
            "read": row["read"],
        }
        for row in rows
...
def format_signed_decimal(value: float) -> str:
    return f"{value:+.4f}"
...
def event_signal_quality(event: EvidenceEvent) -> float:
    quality_base = {
        "primary": 0.90,
        "secondary": 0.72,
        "market": 0.68,
    ...
    lr_values = [value for value in event.likelihood_ratios.values() if value > 0]
    if lr_values:
        mean_log_strength = sum(abs(math.log(value)) for value in lr_values) / len(lr_values)
    else:
        mean_log_strength = 0.0
    clarity_bonus = min(mean_log_strength / 2.0, 0.20)
    dependency_penalty = min(sum(event.dependency_factors.values()) * 0.03, 0.18)
    return clamp_unit_interval(
        quality_base * event.confidence + clarity_bonus - dependency_penalty
    )
...
def kalman_filter_market_odds(
    ...
    for signal in signals:
        ...
        observation_variance = config.observation_noise_floor / max(
            signal.reliability_weight, 0.05
        )
...
def run_synthetic_bayesian_backtest(
    runs: int = 48, events_per_run: int = 8, seed: int = 2027
) -> BayesianBacktestResult:
    rng = random.Random(seed)
    hypotheses = ResonanceBayesianTracker.default_hypotheses()
    rows = []
    for run_index in range(runs):
        true_hypothesis = hypotheses[run_index % len(hypotheses)].key
        tracker = ResonanceBayesianTracker(
            hypotheses, verbose=False, detailed_console=False
        )
        for event_index in range(events_per_run):
            likelihood_ratios = {}
            for hypothesis in hypotheses:
                if hypothesis.key == true_hypothesis:
                    likelihood_ratios[hypothesis.key] = rng.uniform(1.25, 2.50)
                else:
                    likelihood_ratios[hypothesis.key] = rng.uniform(0.45, 1.05)
            tracker.add_event(
                EvidenceEvent.from_date(
                    date_str=f"2026-06-{event_index + 1:02d}",
                    title=f"synthetic event {event_index + 1}",
                    quality="secondary",
                    source="synthetic generator",
                    rationale="Synthetic calibration event with known hidden label.",
                    likelihood_ratios=likelihood_ratios,
                    confidence=rng.uniform(0.55, 0.90),
                )
            )
        predicted_probability = clamp_probability(tracker.posteriors[true_hypothesis])
        brier = (1 - predicted_probability) ** 2
        log_loss = -math.log(predicted_probability)
        rows.append(
            {
                "run": run_index + 1,
                ...
                "read": backtest_probability_phrase(predicted_probability),
            }
        )

    calibration_rows = bayesian_backtest_calibration_rows(rows)
    avg_brier = sum(cast(float, row["brier"]) for row in rows) / len(rows)
    avg_log_loss = sum(cast(float, row["log_loss"]) for row in rows) / len(rows)
    summary = (
        f"Synthetic Bayesian backtest average Brier={avg_brier:.3f}, "
        f"average log-loss={avg_log_loss:.3f}; {backtest_loss_phrase(avg_brier)}."
    )
    return BayesianBacktestResult(rows, calibration_rows, summary)


def print_bayesian_backtest_result(
    result: BayesianBacktestResult, max_rows: int = 12
) -> None:
    print("\n=== Synthetic Bayesian Backtest ===")
    print_console_note(
        "What this checks",
        "Synthetic events are generated with a known hidden hypothesis. Brier and log-loss "
        "measure whether the Bayesian engine assigns high probability to that hidden truth.",
    )
    print(result.summary)
    print("\nRun metrics:")
    displayed_rows = result.rows[:max_rows]
    print_compact_rows(compact_backtest_rows(displayed_rows))
    if len(result.rows) > len(displayed_rows):
        print_console_note(
            "Console trim",
            f"Showing {len(displayed_rows)} of {len(result.rows)} runs; export a report for the full table.",
        )
    if result.calibration_rows:
        print("\nCalibration bins:")
        print_console_note(
            "How to read it",
            "`mean_prediction` should be high because every row tracks the true hidden label. "
            "Low bins reveal under-confidence or noisy synthetic evidence.",
        )
        print_compact_rows(result.calibration_rows)


def bayesian_backtest_calibration_rows(
    rows: Sequence[Mapping[str, object]]
) -> list[dict[str, object]]:
    ...
    for low, high in bins:
        values = [
            cast(float, row["predicted_probability"])
            for row in rows
            if low <= cast(float, row["predicted_probability"]) < high
            or (high == 1.0 and cast(float, row["predicted_probability"]) == 1.0)
        ]
        if not values:
            continue
        calibration_rows.append(
            {
                ...
                "count": len(values),
                "mean_prediction": round(sum(values) / len(values), 4),
                "empirical_truth_rate": 1.0,
                "read": calibration_bin_phrase(sum(values) / len(values)),
            }
        )
...
def spread_phrase(spread: float | None) -> str:
    if spread is None:
        return "no bid/ask spread supplied"
    if spread <= 0.02:
        return "tight spread"
    if spread <= 0.08:
...
def source_reliability_phrase(reliability: float) -> str:
    if reliability < 0.35:
        return "low learned reliability"
    if reliability < 0.55:
        return "cautious learned reliability"
    if reliability < 0.75:
        return "moderate learned reliability"
    return "strong learned reliability"


def posterior_predictive_phrase(gap: float) -> str:
    if gap < 0.06:
        return "predictive fit looks tight"
    if gap < 0.14:
        return "predictive fit is acceptable"
    if gap < 0.25:
        return "predictive fit is loose"
    return "predictive fit is poor"


def backtest_probability_phrase(probability: float) -> str:
    if probability >= 0.80:
        return "true label recovered strongly"
    if probability >= 0.60:
        return "true label recovered moderately"
    if probability >= 0.40:
        return "true label only weakly recovered"
    return "true label missed"


def backtest_loss_phrase(avg_brier: float) -> str:
    if avg_brier < 0.05:
        return "excellent synthetic calibration"
    if avg_brier < 0.12:
        return "good synthetic calibration"
    if avg_brier < 0.22:
        return "mixed synthetic calibration"
    return "weak synthetic calibration"


def calibration_bin_phrase(mean_prediction: float) -> str:
    if mean_prediction >= 0.80:
        return "well concentrated on true label"
    if mean_prediction >= 0.60:
        return "usable but not decisive"
    return "under-confident for known-truth synthetic runs"
...
def build_demo_tracker(
    verbose: bool = True, detailed_console: bool = False
) -> ResonanceBayesianTracker:
    tracker = ResonanceBayesianTracker(
        verbose=verbose, detailed_console=detailed_console
    )
...
def print_console_dashboard(
    tracker: ResonanceBayesianTracker,
    chaos_result: ChaosStabilityResult,
    sensitivity_result: SensitivityResult,
    posterior_predictive_result: PosteriorPredictiveResult,
    synthetic_validation_result: SyntheticValidationResult,
    bayesian_backtest_result: BayesianBacktestResult,
) -> None:
    print("\n=== Vatican / AI Narrative Tracker ===")
    print_console_note(
        "Bottom line",
        "This is a subjective evidence ledger, not a proof engine. Read high probabilities "
        "as model beliefs after the current assumptions.",
    )

    print("\nCurrent beliefs:")
    print_compact_rows(compact_posterior_rows(tracker.posterior_table()))

    print("\nMarket adjustment:")
    pool_rows = tracker.market_opinion_pool_table()
    if pool_rows:
        print_compact_rows(compact_pool_rows(pool_rows))
    else:
        print("  No market rows affected the model.")

    print("\nReliability and stability:")
    print_console_note("Stability", f"{chaos_result.classification}; {chaos_result.confidence}.")
    print_console_note("Sensitivity", sensitivity_result.summary)
    print_console_note("Predictive check", posterior_predictive_result.summary)

    print("\nValidation checks:")
    print_console_note("Dynamics", synthetic_validation_result.summary)
    print_console_note("Bayesian backtest", bayesian_backtest_result.summary)

    print("\nNext inspection:")
    print_console_note(
        "Tip",
        "Run with --detailed for event-by-event diagnostics, or --report PATH for the full audit trail.",
    )


def run_cli(argv: Sequence[str] | None = None) -> None:
    parser = argparse.ArgumentParser(
        description="Run the Vatican / AI narrative Bayesian tracker demo."
    )
    parser.add_argument(
        "--demo",
        action="store_true",
        help="Run the built-in demo ledger. This is also the default when no input file is supplied.",
    )
    parser.add_argument("--report", help="Write a Markdown report to this path.")
    parser.add_argument("--recurrence", help="Write a recurrence matrix CSV to this path.")
    parser.add_argument("--phase-space", help="Write a phase-space portrait image to this path.")
    parser.add_argument("--timeline", help="Write the posterior timeline image to this path.")
    parser.add_argument(
        "--quiet",
        action="store_true",
        help="Suppress verbose event-by-event console output where possible.",
    )
    parser.add_argument(
        "--detailed",
        action="store_true",
        help="Print full diagnostic tables in addition to compact summaries.",
    )
    parser.add_argument(
        "--skip-interactive-plot",
        action="store_true",
        help="Do not open the timeline plot when no --timeline path is supplied.",
    )
    args = parser.parse_args(argv)

    tracker = build_demo_tracker(
        verbose=(not args.quiet and args.detailed), detailed_console=args.detailed
    )
    if not args.quiet and args.detailed:
        tracker.show_source_reliability_summary()
        tracker.show_polymarket_summary()
    chaos_result = tracker.run_chaos_stability_analysis()
    sensitivity_result = tracker.run_sensitivity_analysis()
    pca_result = tracker.run_narrative_pca()
    tracker.run_temporal_pca()
    posterior_predictive_result = tracker.run_posterior_predictive_checks()
    synthetic_validation_result = run_synthetic_dynamics_validation()
    if not args.quiet and args.detailed:
        print_synthetic_validation_result(synthetic_validation_result)
    bayesian_backtest_result = run_synthetic_bayesian_backtest()
    if not args.quiet and args.detailed:
        print_bayesian_backtest_result(bayesian_backtest_result)
    if not args.quiet and not args.detailed:
        print_console_dashboard(
            tracker,
            chaos_result,
            sensitivity_result,
            posterior_predictive_result,
            synthetic_validation_result,
            bayesian_backtest_result,
        )
    ...
    if args.report:
        tracker.export_report(
            ...
            posterior_predictive_result=posterior_predictive_result,
            bayesian_backtest_result=bayesian_backtest_result,
        )
    if args.recurrence:
        tracker.export_recurrence_matrix(args.recurrence)
    if args.phase_space:
        tracker.plot_phase_space(args.phase_space)
    if args.timeline:
        tracker.plot_probability_timeline(args.timeline)
    elif not args.skip_interactive_plot and not args.quiet:
